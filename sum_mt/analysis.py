...
class SUMStat:
    ...
    def get_avg_score(self, metric_name):
        ...
        for doc_id in self.data:
            ...
            for sys_name in sys_summs:
                ...
                sys_scores[sys_name].append(sys_summs[sys_name]['scores'][metric_name])

        avg_scores = []
        for sys_name in sys_scores:
            avg_s = np.mean(sys_scores[sys_name])
            print(sys_name, 'len:', len(sys_scores[sys_name]), 'avg:', avg_s)
            avg_scores.append(avg_s)
        
        print()
        print('all_avg:', np.mean(avg_scores))
